...
import { getMovieDetails, getCredits, getImageUrl, Movie, Credits } from '@/lib/tmdb';
import Image from 'next/image';
import Link from 'next/link';
import { useState, useEffect } from 'react';
import Loading from '@/components/Loading';

// As funções de ajuda podem ficar aqui ou ser importadas
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const creditsData = await getCredits('movie', movieId);
        setMovie(movieData);
  ...
  return (
    <div className="text-white">
      ...
      <div className="relative w-full h-[50vh] md:h-[60vh]">
        <Image src={getImageUrl(movie.backdrop_path, 'original')} alt={movie.title || 'Backdrop'} fill priority className="object-cover object-top w-full h-full" />
      ...
      <div className="container mx-auto px-4 md:px-16 pb-16 -mt-32 relative z-10">
        <div className="flex flex-col md:flex-row items-center md:items-start text-center md:text-left md:space-x-8">
          ...
          <div className="flex-shrink-0 w-48 md:w-64">
            <Image src={getImageUrl(movie.poster_path, 'w500')} alt={movie.title || 'Poster'} width={500} height={750} className="rounded-lg shadow-2xl" />
          </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {credits.cast.slice(0, 12).map((actor) => (
                <div key={actor.id} className="text-center">
                  <Image src={getImageUrl(actor.profile_path, 'w500')} alt={actor.name} width={200} height={300} className="rounded-lg object-cover w-full aspect-[2/3] mb-2 bg-gray-800" />
